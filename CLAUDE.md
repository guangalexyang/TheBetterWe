...
1. **Phase 1 (current)** — iOS app + Node/Express server + PostgreSQL; user auth, family setup, Point System (kids + rules + points + redemptions + goals), personal + family TODOs/Done, Highlight of the Day
   ...
   - ✅ Family QR invite — `InviteSheet` (QR display via `CIQRCodeGenerator` 10× scaled, share via `UIActivityViewController`, save to photos with green success feedback, refresh invalidates old code immediately); `QRScannerView` (full-screen `DataScannerViewController`, green sweep line animation, album scan via `PhotosPicker` + `VNDetectBarcodesRequest`); `JoinFamilyView` (gradient family banner, display name pre-filled from `AuthService.displayName`, role chips Dad/Mom/Kid/Other, custom text field when Other selected); deep link format `thebetterwe://join?code=<uuid>`; 4 server endpoints (`GET/POST /:id/invite-code`, `GET /by-invite/:code`, `POST /join`); `familyInviteQR` toggle **on**
