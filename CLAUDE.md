...
1. **Phase 1 (current)** — iOS app + Node/Express server + PostgreSQL; user auth, family setup, Point System (kids + rules + points + redemptions + goals), personal + family TODOs/Done, Highlight of the Day
   ...
   - ✅ Volcengine ASR — full server proxy at `/asr/stream` WebSocket; `ASRBackend` protocol with `AppleBackend` + `VolcengineBackend`; status label shows `聆听中[火山]` or `聆听中[Apple]`; falls back to Apple automatically; **pending:** `fly secrets set VOLCENGINE_ASR_APP_ID VOLCENGINE_ASR_TOKEN` + deploy
